...
const apiBaseUrl = "https://adocao-animal-api.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          ...
          <li>✓ Controle completo de Adotante</li>
          <li>✓ Controle completo de Processo Adocao</li>
          <li>✓ Controle completo de Entrevista Adocao</li>
        </ul>
      </div>
...
function PainelAnimal({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["especie", "raca", "nome", "idade", "porte", "sexo", "vacinado", "castrado", "descricao", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const editar = (item) => {
    const f = {};
    ["especie", "raca", "nome", "idade", "porte", "sexo", "vacinado", "castrado", "descricao", "status"].forEach(k => { f[k] = item[k] ?? ""; });
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["especie"] ?? "Animal")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/animals/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["especie"] ?? "Animal")}</div>
            ...
            <div className="item-field"><b>idade</b><span>{item["idade"] ?? "—"}</span></div>
            <div className="item-field"><b>porte</b><span>{item["porte"] ?? "—"}</span></div>
            <div className="item-field"><b>sexo</b><span>{item["sexo"] ?? "—"}</span></div>
            <div className="item-field"><b>vacinado</b><span>{item["vacinado"] != null ? (item["vacinado"] ? "Sim" : "Não") : "—"}</span></div>
            <div className="item-field"><b>castrado</b><span>{item["castrado"] != null ? (item["castrado"] ? "Sim" : "Não") : "—"}</span></div>
      ...
      <select value={form["porte"] ?? "MINI"} onChange={e => setForm({...form, porte: e.target.value})}>
      ...
      <select value={form["sexo"] ?? "MASCULINO"} onChange={e => setForm({...form, sexo: e.target.value})}>
      ...
      <label className="field-label checkbox-label">
        <input type="checkbox" checked={form["vacinado"] === true || form["vacinado"] === "true"} onChange={e => setForm({...form, vacinado: e.target.checked})} />
        vacinado
      </label>
      <label className="field-label checkbox-label">
        <input type="checkbox" checked={form["castrado"] === true || form["castrado"] === "true"} onChange={e => setForm({...form, castrado: e.target.checked})} />
        castrado
      </label>
...
function PainelAdotante({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "email", "telefone", "endereco", "cidade", "renda", "moradia", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/adotantes", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/adotantes/" + editId : apiBaseUrl + "/api/adotantes";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "email", "telefone", "endereco", "cidade", "renda", "moradia"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/adotantes/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Adotante")}</div>
            ...
            <div className="item-field"><b>moradia</b><span>{item["moradia"] ?? "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Adotante</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">nome</label>
      <input type="text" value={form["nome"] ?? ""} onChange={e => setForm({...form, nome: e.target.value})} />
      ...
      <label className="field-label">moradia</label>
      <input type="text" value={form["moradia"] ?? ""} onChange={e => setForm({...form, moradia: e.target.value})} />
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
              </div>
...
function PainelProcessoAdocao({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [adotanteList, setAdotanteList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["animalId", "adotanteId", "dataInicio", "status", "responsavel", "observacoes", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/processoadocaos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    ...
    fetch(apiBaseUrl + "/api/adotantes", { headers: authHeaders(token) }).then(r => r.json()).then(setAdotanteList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/processoadocaos/" + editId : apiBaseUrl + "/api/processoadocaos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["animalId", "adotanteId", "dataInicio", "status", "responsavel", "observacoes"].forEach(k => { f[k] = item[k] ?? ""; });
    ["animal", "adotante"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/processoadocaos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["status"] ?? "ProcessoAdocao")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/processoadocaos/" + item.id, {
                      method: "PUT", headers: { "Content-Type": "application/json", ...authHeaders(token) },
                      body: JSON.stringify({ ...item, status: e.target.value }),
                    }).then(carregar);
                  }}>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["status"] ?? "ProcessoAdocao")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>animalId</b><span>{item["animalId"] ?? "—"}</span></div>
            <div className="item-field"><b>adotanteId</b><span>{item["adotanteId"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>animal</b><span>{item.animalId ? ((animalList.find(o => o.id === item.animalId) || {}).especie ?? ("#" + item.animalId)) : "—"}</span></div>
            <div className="item-field"><b>adotante</b><span>{item.adotanteId ? ((adotanteList.find(o => o.id === item.adotanteId) || {}).nome ?? ("#" + item.adotanteId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} ProcessoAdocao</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
      ...
      <label className="field-label">adotanteId</label>
      <input type="number" value={form["adotanteId"] ?? ""} onChange={e => setForm({...form, adotanteId: e.target.value})} />
      ...
      <select value={form["animal"] ?? ""} onChange={e => setForm({...form, animal: e.target.value})}>
        ...
        {(animalList || []).map(o => (<option key={o.id} value={o.id}>{o.especie ?? ("#" + o.id)}</option>))}
      </select>
      <label className="field-label">adotante</label>
      <select value={form["adotante"] ?? ""} onChange={e => setForm({...form, adotante: e.target.value})}>
        <option value="">Selecione...</option>
        {(adotanteList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelEntrevistaAdocao({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [busca, setBusca] = React.useState("");
  const [pagina, setPagina] = React.useState(1);
  const [processoAdocaoList, setProcessoAdocaoList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["processoAdocaoId", "data", "tipo", "resultado", "pontuacao", "observacoes", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/entrevistaadocaos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/processoadocaos", { headers: authHeaders(token) }).then(r => r.json()).then(setProcessoAdocaoList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/entrevistaadocaos/" + editId : apiBaseUrl + "/api/entrevistaadocaos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["processoAdocaoId", "data", "tipo", "resultado", "pontuacao", "observacoes"].forEach(k => { f[k] = item[k] ?? ""; });
    ["processoAdocao"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/entrevistaadocaos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["tipo"] ?? "EntrevistaAdocao")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>processoAdocaoId</b><span>{item["processoAdocaoId"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>pontuacao</b><span>{item["pontuacao"] ?? "—"}</span></div>
            <div className="item-field"><b>observacoes</b><span>{item["observacoes"] ?? "—"}</span></div>
            <div className="item-field"><b>processoAdocao</b><span>{item.processoAdocaoId ? ((processoAdocaoList.find(o => o.id === item.processoAdocaoId) || {}).status ?? ("#" + item.processoAdocaoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} EntrevistaAdocao</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">processoAdocaoId</label>
      <input type="number" value={form["processoAdocaoId"] ?? ""} onChange={e => setForm({...form, processoAdocaoId: e.target.value})} />
      ...
      <input type="number" value={form["pontuacao"] ?? ""} onChange={e => setForm({...form, pontuacao: e.target.value})} />
      <label className="field-label">observacoes</label>
      <textarea rows={3} value={form["observacoes"] ?? ""} onChange={e => setForm({...form, observacoes: e.target.value})} />
      <label className="field-label">processoAdocao</label>
      <select value={form["processoAdocao"] ?? ""} onChange={e => setForm({...form, processoAdocao: e.target.value})}>
        <option value="">Selecione...</option>
        {(processoAdocaoList || []).map(o => (<option key={o.id} value={o.id}>{o.status ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Adocao Animal<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Animal" ? " active" : "")} onClick={() => setAba("Animal")}><span className="nav-ico">🐾</span>Animal</button>
        <button className={"nav-btn" + (aba === "Adotante" ? " active" : "")} onClick={() => setAba("Adotante")}><span className="nav-ico">🗂️</span>Adotante</button>
        <button className={"nav-btn" + (aba === "ProcessoAdocao" ? " active" : "")} onClick={() => setAba("ProcessoAdocao")}><span className="nav-ico">🧾</span>ProcessoAdocao</button>
        <button className={"nav-btn" + (aba === "EntrevistaAdocao" ? " active" : "")} onClick={() => setAba("EntrevistaAdocao")}><span className="nav-ico">🗂️</span>EntrevistaAdocao</button>
      ...
      {aba === "Adotante" && <PainelAdotante token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "ProcessoAdocao" && <PainelProcessoAdocao token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "EntrevistaAdocao" && <PainelEntrevistaAdocao token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
